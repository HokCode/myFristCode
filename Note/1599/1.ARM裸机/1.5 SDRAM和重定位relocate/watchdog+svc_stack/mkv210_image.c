...
int main (int argc, char *argv[])
{
	...
	// 3. 读源bin到buffer
	// 3.1 打开源bin
	fp = fopen(argv[1], "rb"); //r b,用二进制的只读方式打开文件
	if( fp == NULL)
	{
	...
	count = (fileLen < (IMG_SIZE - SPL_HEADER_SIZE))	
		? fileLen : (IMG_SIZE - SPL_HEADER_SIZE);		//取小于16K的部分
		
	// 3.4 buffer[0~15]存放"S5PC110 HEADER  "
	memcpy(&Buf[0], SPL_HEADER, SPL_HEADER_SIZE);	//占位
	...
	for(i = 0, checksum = 0; i < IMG_SIZE - SPL_HEADER_SIZE; i++)	//扫描整个bin区。
		checksum += (0x000000FF) & *a++;	//a指向bin部分，一个个字节来增加。
	
	// 4.2 将校验和保存在buffer[8~15]
	a = Buf + 8;							// Buf是210.bin的起始地址，+8表示向后位移2个字，也就是说写入到第3个字 checksum
